...
class Dispatch:
    ...
    def fast_dispatch(network):
        ...
        marked_edges = []
        for node_idx in range(network.num_tps()):
            distance_matrix[node_idx], predecessor_graphs[node_idx] = Dijkstra.dijkstra_wrapper(network, node_idx, dispatch=True)
            for successor_idx, weight in enumerate(network.successor_edges[node_idx]):
                network.successor_edges[node_idx][successor_idx] = weight + \
